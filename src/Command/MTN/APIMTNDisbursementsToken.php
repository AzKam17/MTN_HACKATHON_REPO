...
#[AsCommand(
    name: 'mtn:disbursements:token',
    description: 'Generer un token pour l\'API de disbursement',
)]
class APIMTNDisbursementsToken extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $basicAuth = base64_encode($userDisbursements->getValue() . ':' . $apiKeyDisbursements->getValue());
        ...
        if ($client->getResponse()->getStatusCode() == 200) {
            ...
            $apiCollectionToken = $this->em->getRepository(Params::class)->findOneBy(['lib' => 'TOKEN_DISBURSEMENTS']);
            //If not exist, create it
            if (!$apiCollectionToken) {
                $apiCollectionToken = new Params();
                $apiCollectionToken->setLib('TOKEN_DISBURSEMENTS');
            }
